...
const sliderArray = [
  {
    id: 1,
    image: slider1,
    text1: "Don't miss amazing",
    text2: "grocery deals",
    text3: "Sign up for the daily newsletter",
  },
  {
    id: 2,
    image: slider2,
    text1: "Fresh Vegetables",
    text2: "Big discount",
    text3: "Sign up for the daily newsletter",
  },
];
        ...
        {sliderArray.map(slide => (
          <SwiperSlide key={slide.id}>
            <div className="relative flex items-center justify-center p-6 rounded-3xl">
              <img
                src={slide.image}
                className="bg-cover bg-center bg-no-repeat object-cover rounded-3xl w-full h-full min-h-[350px]"
              />
              <section className="absolute top-1/2 bottom-1/2 left-8 sm:left-20 flex flex-col justify-center gap-2 items-start">
                <p className="pt-[20px] font-semibold text-2xl sm:text-3xl md:text-4xl lg:text-[72px]">
                  {slide.text1}
                </p>
                <p className="pt-[20px] font-semibold text-2xl sm:text-3xl md:text-4xl lg:text-[72px]">
                  {slide.text2}
                ...
                <div className="bg-white mt-[20px] h-12 bottom-32 text-[10px] sm:text-[16px] left-20 rounded-3xl flex gap-2 justify-between items-center">
                  ...
                  />
                  <button className="bg-primary-default h-12 rounded-3xl p-4 flex items-center text-white">
                    Subscribe
                  </button>
                </div>
              </section>
