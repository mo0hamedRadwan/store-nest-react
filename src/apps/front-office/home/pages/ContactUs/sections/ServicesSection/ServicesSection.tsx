      ...
      <section className="my-10">
        <div className="mx-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-6">
          {benefits.map((benefit, index) => (
            <div key={index} className="flex items-center bg-[#F4F6FA] p-6 rounded-lg shadow-md">
              <div className="max-w-[60px] duration-[0.3s] mr-5">
                <img src={benefit.icon} alt={benefit.title} className="max-w-full" />
              </div>
              <div>
                <h4 className="text-xl font-semibold text-gray-900">{trans(benefit.title)}</h4>
